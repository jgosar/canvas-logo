...
export class ExecuteCommandReducer implements Reducer<LogoStoreState, string> {
  ...
  private formatCommandTextWithArgs(commandText: string, args: string[]): string {
    return args.reduce((commandText, arg, argIndex) => replaceAll(commandText, `@ARG${argIndex}@`, arg), commandText);
  }
  ...
  private tokenizeCommand(command: string): string[] {
    ...
    return processedCommand
      .split(' ')
      .map((token) => (token.startsWith('bracketedExpression-') ? bracketedExpressions[token.split('-')[1]] : token));
  }
}
